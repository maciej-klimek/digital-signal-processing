...
% Okno Hamminga
N = length(x);  % d³ugoæ sygna³u
Mlen = 240;  % d³ugoæ okna Hamminga (liczba próbek)
Mstep = 180;  % przesuniêcie okna w czasie (liczba próbek)
Np = 10;  % rz¹d filtra predykcji
gdzie = Mstep + 1;  % pocz¹tkowe polo¿enie pobudzenia dwiêcznego
...
for nr = 1:Nramek
    ...
    bx = bx - mean(bx);  % usuñ wartoæ redni¹
    for k = 0:Mlen-1
    ...
    if (rmax > 0.35 * r(1)) 
        ...
        T = 0; 
    end  % g³oska dwiêczna/bezdwiêczna?
    ...
    H = freqz(1, [1; a]);  % oblicz jego odp. czêstotliwociow¹
    ...
    if (T ~= 0) 
        gdzie = gdzie - Mstep; 
    end  % 'przenie' pobudzenie dxwiêczne
    for n = 1:Mstep
        ...
        if (T == 0)
            ...
            if (n == gdzie) 
                pob = 1; 
                gdzie = gdzie + T;  % pobudzenie dwiêczne
            else 
                pob = 0; 
            end
        end
        ...
        bs = [ss(n) bs(1:Np-1)];  % przesuniêcie bufora wyjciowego
    end
