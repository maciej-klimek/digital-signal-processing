...
%     subplot(413); plot(abs(H)); title('widmo filtra traktu głosowego');
    if (T ~= 0)
        ...
        figure;
        subplot(2, 1, 1); plot(resztkowy); title('Sygnał resztkowy');
        df = (fpr / length(resztkowy)) / 2; 
        f = df * (0 : length(resztkowy) - 1);
        Reszt = fft(resztkowy);
        [~, maxpos] = max(Reszt);
        T = 1 / (2 * pi * f(maxpos));
        subplot(2, 1, 2); plot(f, Reszt); title('Widmo sygnału resztkowego');
    end
    ...
    for n = 1 : Mstep
        ...
        if (T == 0)
            ...
            pob = resztkowy(n); % wykorzystanie sygnału resztkowego
        end
